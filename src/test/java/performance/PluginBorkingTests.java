...
public class PluginBorkingTests {
	...
	public void testGet200Ok() throws Exception {
		GenericUrl url = new GenericUrl(
				"http://" + InetAddress.getLocalHost().getHostAddress() + ":" + port + "/" + defaultFileName);
	...
	@Test
	public void borkTest() throws Exception {
		GenericUrl borkUrl = new GenericUrl(
				"http://" + InetAddress.getLocalHost().getHostAddress() + ":" + port + "/" + "bork.html");
		HttpRequest borkRequest = requestFactory.buildGetRequest(borkUrl);

		try {
			HttpResponse borkResponse = borkRequest.execute();
		} catch (SocketTimeoutException e) {
			System.out.println("BORK request timed out!");
		}
